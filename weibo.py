...
from weibomodel import SimilarityModule, DetectionModule
# Configs
DEVICE = "cuda:0"
NUM_WORKER = 1
BATCH_SIZE = 64
...
def train():
    ...
    for epoch in range(num_epoch):
        ...
        for i, (text, image, label) in tqdm(enumerate(train_loader)):  # tqdm是进度条库，可以在python的长循环中添加一个进度提示信息
            ...
            loss_similarity = loss_func_similarity(text_aligned_4_task1, image_aligned_4_task1,
                                                   similarity_label_1)
            ...
            corrects_pre_detection += pre_label_detection.eq(
                label.view_as(pre_label_detection)).sum().item()
